---
title: "Reproduced Correlation Coefficient Matrix"
output:
...
This notebook contains the R scripts for calculating the reproduced correlation coefficient (RCC), generating a heatmap of the restulting matrix, and translating the final data matrix for network analysis in Cytoscape.
...
```{r}
# Eigenvalue by Kaiser-Guttman criterion - want EV (PCs) above the average
# broken stick model - want EV (PCs) > broken stick model
# http://en.proft.me/2016/11/15/principal-component-analysis-pca-r/
ev <- PCA.model$sdev^2
n = length(ev)
...
RCC <- (Meanestmat %*% t(Meanestmat))/(normmem %*% t(normmem))
...
heat.mat <- data.matrix(RCC[,1:ncol(RCC)])  # transform columns into a matrix
