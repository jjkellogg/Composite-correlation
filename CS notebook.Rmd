---
title: "Composite Coefficient Correlations"
output:
...
This notebook contains the R scripts for calculating the composite coefficient (CC), generating a heatmap of the restulting matrix, and translating the final data matrix for network analysis in Cytoscape.
...
CC <- (Meanestmat %*% t(Meanestmat))/(normmem %*% t(normmem))
...
```{r}
rnames <- RAWdata[,1]                       # assign labels in column 1 to "rnames"
heat.mat <- data.matrix(CC[,1:ncol(CC)])  # transform columns into a matrix
